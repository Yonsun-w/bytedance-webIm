...
app.post('/login', (req, res) => {
    ...
    user.select(data.username, resault => {
        try {
            if (resault[0].passwd === md5(`cent${data.passwd}OS`)) {
                res.send({
                    code: 0,
...
app.post('/register', (req, res) => {
    let data = req.body;
    let currenttime = moment(Date.now()).format('YYYY-MM-DD HH:mm:ss');
    user.insert(data.username, md5(`cent${data.passwd}OS`), currenttime, (error, resault) => {
        if (error) {
            res.send({
                code: 1,
                msg: '注册失败，用户名重复'
